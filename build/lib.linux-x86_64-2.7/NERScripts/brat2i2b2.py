# Copyright (c) er outreach and education 2016-2017 Fred Hutchinson Cancer Research Center
#
...
{DIR}/i2b2_data/train/           <------The output dir
                  |-conceptoot
           webroot/brat-v1.3_Crunchy_Frog/data/.retrain_demo"
            webroot/brat-v1.3_Cruncy_Frog/data/.retrain_i2b2"
                  |    |-etc
                  |-txt
                  |    |-file1.txt
                  |    |-file2.txt
...
in_dir = "/home/ubuntu/webroot/brat-v1.3_Crunchy_Frog/data/Emily/BreastPath2015"
out_dir = "/home/ubuntu/webroot/hutchner/Datasets/BreastPathElements"

#out_dir = "C:/users/esilgard/Desktop/brat/phi_only_validation_batch1_in_i2b2"
nlp = spacy.load('en')
...
for dpath in doc_paths:
    ...
    ann_file = os.path.join(in_dir, dpath.split(os.path.sep)[-1].split(".")[0] + ".ann")
...
for did, txt in txt_dict.items():
    ...
    for i, single_ann in enumerate(curr_doc_annotation_split):
        ann_pieces = single_ann.strip().split("\t")
        if len(ann_pieces)>1:
            label = ann_pieces[1].split()[0].lower()
    ...
    #print ('\n'.join([str(x) for x in ordered_annotations])+ '\n')
    ...
    for sent_num, sent in enumerate(proc_sentences):
        ...
        for tok in sent:
            if not re.match(r'^\s*$', tok.orth_): # if the token is just whitespace, dont count or include it
                tok_count+=1
		#print ('tok count: ' + str(tok.idx) + ' ' + tok.orth_ + ' current ann idx: ' + str(ordered_annotations[ann_idx]))		
                if ann_idx < len(ordered_annotations):
                    # check for a new annotation start FIRST 
                    # this is so you don't skip over an annotation that directly follows another
                    if tok.idx > ordered_annotations[ann_idx][1]:
                        ann_idx += 1
                    if ann_idx < len(ordered_annotations) and tok.idx >= ordered_annotations[ann_idx][0] and tok.idx < ordered_annotations[ann_idx][1]:
        ...
        if(sent_tokens): print sent_tokens
